...
static  uint8_t buff_pool[0x10000] __attribute__((aligned(32)));
...
static void word_swap_hex(uint8_t *hex, int len)
{
	int      i;
	uint8_t  val8;
...
static void swap_registers(uint32_t * reg_base, int reg_cnt)
{
	int       i;
...
static int read_test_vector(KAT_T *t)
{
	int         i, count;
	uint32_t    *key_ptr;
...
static void dump_buff_hex(uint8_t *pucBuff, int nBytes)
{
