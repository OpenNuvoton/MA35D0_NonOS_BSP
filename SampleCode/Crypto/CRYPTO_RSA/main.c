/**************************************************************************//**
 * @file     main.c
 * @brief    This sample program demonstrates the Crypto RSA 1024/2048/3072/4096
 *           bits encrypt and decrypt.
 *
...
struct rsa_tv_t {
	...
	uint8_t N[520];
	int elen;
	uint8_t E[520];   /* public key  */
	int dlen;
	uint8_t D[520];   /* private key */
} ;
...
void print_data(char *str, char *buff, int len)
{
	...
	sysprintf("%s: ", str);
	for (i = 0; i < len; i++)
		sysprintf("%02x", (unsigned char)buff[i]);
	sysprintf("\n");
}
...
int32_t main(void)
{
	...
	tv = nc_ptr(rsa_vectors);

	sysprintf("+-----------------------------------------+\n");
	sysprintf("|   Crypto RSA Encrypt/Decrypt Demo       |\n");
	sysprintf("+-----------------------------------------+\n");

	for (i = 0; i < sizeof(rsa_vectors)/sizeof(struct rsa_tv_t); i++)
	{
		sysprintf("RSA %d test ==>\n", tv[i].bitlen);
		print_data("RSA plain text data ==>  ", tv[i].M, tv[i].mlen);

		sysprintf("RSA %d encrypt ==> ", tv[i].bitlen);
		t0 = EL0_GetCurrentPhysicalValue();
		ret = RSA_Encrypt(CRPT, tv[i].bitlen, tv[i].M, tv[i].mlen, tv[i].E, tv[i].elen,
						  tv[i].N, tv[i].nlen, result, &olen);

		if (ret != 0)
		{
			sysprintf("RSA encrypt failed! %d\n", ret);
			sysprintf("CRPT->RSA_CTL = 0x%x\n", CRPT->RSA_CTL);
			sysprintf("CRPT->RSA_STS = 0x%x\n", CRPT->RSA_STS);
			sysprintf("CRPT->INTSTS = 0x%x\n", CRPT->INTSTS);
			while (1);
		}

		t0 = (EL0_GetCurrentPhysicalValue() - t0) / 12;
		sysprintf("%d us\n", t0);

		print_data("RSA encrypt output ==>\n", result, olen);

		sysprintf("RSA %d decrypt ==> ", tv[i].bitlen);
		t0 = EL0_GetCurrentPhysicalValue();
		ret = RSA_Encrypt(CRPT, tv[i].bitlen, result, olen, tv[i].D, tv[i].dlen,
						  tv[i].N, tv[i].nlen, result, &olen);
		if (ret != 0)
		{
			sysprintf("RSA decrypt failed! %d\n", ret);
			while (1);
		}
