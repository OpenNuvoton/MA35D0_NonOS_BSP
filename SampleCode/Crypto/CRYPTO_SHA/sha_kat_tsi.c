...
static uint8_t  g_sha_digest[1024] __attribute__((aligned(32)));
...
static void do_swap(uint8_t *buff, int len)
{
	int       i;
	uint8_t   val8;
...
static int do_compare(uint32_t *output, uint32_t *expect, int cmp_len)
{
	int   i;
...
int  SHA_KAT_TSI()
{
	...
	sysprintf("\n\n+---------------------------------------+\n");
	sysprintf("|  MA35D0 SHA Known-Answer-Test         |\n");
	sysprintf("+---------------------------------------+\n");
