...
void SYS_Init(void)
{
    ...
    /* Select clock source of SPI1 and SPI0 */
...
void SPI_Init(void)
{
    ...
    /*---------------------------------------------------------------------------------------------------------*/
    /* Configure SPI1 */
    /* Configure SPI1 as a master, SPI clock rate 2MHz,
...
void SpiLoopTest_WithPDMA(void)
{
    ...
    PDMA_SetTransferAddr(PDMA2, SPI_MASTER_TX_DMA_CH, ptr_to_u32(pau32MasterToSlaveTestPattern), PDMA_SAR_INC, ptr_to_u32(&SPI1->TX), PDMA_DAR_FIX);
    /* Set request source; set basic mode. */
    PDMA_SetTransferMode(PDMA2, SPI_MASTER_TX_DMA_CH, PDMA_SPI1_TX, FALSE, 0);
    ...
    /* Set source/destination address and attributes */
    PDMA_SetTransferAddr(PDMA2, SPI_MASTER_RX_DMA_CH, ptr_to_u32(&SPI1->RX), PDMA_SAR_FIX, ptr_to_u32(pau32MasterRxBuffer), PDMA_DAR_INC);
    /* Set request source; set basic mode. */
    PDMA_SetTransferMode(PDMA2, SPI_MASTER_RX_DMA_CH, PDMA_SPI1_RX, FALSE, 0);
    /* Single request type. SPI only support PDMA single request type. */
        ...
        while(1)
        {
            ...
            if(u32RegValue & PDMA_INTSTS_TDIF_Msk)
            {
                        ...
                        ((1 << SPI_MASTER_TX_DMA_CH) | (1 << SPI_MASTER_RX_DMA_CH) | (1 << SPI_SLAVE_TX_DMA_CH) | (1 << SPI_SLAVE_RX_DMA_CH)))
                {
                    ...
                    /* Disable SPI1 master's PDMA transfer function */
...
int main(void)
{
    ...
    sysprintf("+--------------------------------------------------------------+\n");
    sysprintf("\n");
    sysprintf("Configure SPI1 as a master and SPI0 as a slave.\n");
    sysprintf("Bit length of a transaction: 32\n");
    sysprintf("The I/O connection for SPI1/SPI0 loopback:\n");
    sysprintf("    SPI1_SS  (PE10) <--> SPI0_SS(PG0)\n    SPI1_CLK(PE11)  <--> SPI0_CLK(PB9)\n");
    sysprintf("    SPI1_MISO(PE13) <--> SPI0_MISO(PL15)\n    SPI1_MOSI(PE12) <--> SPI0_MOSI(PL14)\n\n");
    sysprintf("Please connect SPI1 with SPI0, and press any key to start transmission ...");
    sysgetchar();
    sysprintf("\n");
