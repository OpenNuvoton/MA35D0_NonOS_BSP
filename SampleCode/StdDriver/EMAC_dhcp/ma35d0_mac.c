...
/**
 * @brief Function to register EMAC interrupt handler and enable interrupt
 * @param[in] intf EMAC interface
 *          - \ref EMACINTF0
 *          - \ref EMACINTF1
 * @return Always returns 0.
...
{
    if(intf == EMACINTF0) {
        IRQ_SetHandler(EMAC0_IRQn, EMAC_int_handler0);
        IRQ_Enable(EMAC0_IRQn);
    } else {
        IRQ_SetHandler(EMAC1_IRQn, EMAC_int_handler1);
...
void EMAC_int_handler0(void)
{
    ...
    if(interrupt & EMACDmaRxNormal) {
        TR("%s:: Rx Normal \n", __FUNCTION__);
        EMAC_handle_received_data(EMACINTF0);
    }
...
void EMAC_int_handler1(void)
{
    ...
    if(interrupt & EMACDmaRxNormal) {
        TR("%s:: Rx Normal \n", __FUNCTION__);
        EMAC_handle_received_data(EMACINTF1);
    }
