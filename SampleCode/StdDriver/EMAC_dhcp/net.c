/*************************************************************************//**
 * @file     net.c
 * @brief    Packet processor source file for MA35D0
 *
...
uint8_t g_au8MacAddr[6] = {0};
uint8_t g_au8IpAddr[4] = {0};
...
static void arp_reply(int intf, uint8_t *target_ip, uint8_t *target_mac)
{
    ...
    if(intf == EMACINTF0)
        EMAC0_TransmitPkt(NULL, &au8TxBuf[0], sizeof(ARP_PACKET));
    else
        EMAC1_TransmitPkt(NULL, &au8TxBuf[0], sizeof(ARP_PACKET));
}

static void  udp_send(int intf, uint8_t *pu8SrcMac, uint8_t *pu8SrcIP,
                      uint16_t u16SrcPort, uint8_t *pu8DestMac, uint8_t *pu8DestIP,
                      uint16_t u16DestPort, uint8_t *pu8TxData, uint32_t u32Len)
{
    ...
    if(intf == EMACINTF0)
        EMAC0_TransmitPkt(NULL, &au8TxBuf[0], sizeof(UDP_PACKET) + u32Len);
    else
        EMAC1_TransmitPkt(NULL, &au8TxBuf[0], sizeof(UDP_PACKET) + u32Len);
    // To enable RX IRQ ?
}

int process_rx_packet(int intf, uint8_t *pu8Packet, uint32_t u32Len)
{
    ...
    if (pu8Packet[0] == 0xFF)       /* this is a broadcast packet */
    {
        ...
        {
            arp_reply(intf, arp->su8SenderIP, arp->au8SenderHA);
        }
    ...
    else                        /* this is a multicast or unicast packet */
    {
                ...
                (pu8Packet[34] == 0x08))
        {
            ...
            *(uint16_t *)&au8TxBuf[36] = ~chksum((uint16_t *)&au8TxBuf[34], (u32Len - 34) / 2);
            ...
            if(intf == EMACINTF0)
                EMAC0_TransmitPkt(NULL, au8TxBuf, u32Len);
            else
                EMAC1_TransmitPkt(NULL, au8TxBuf, u32Len);
...
int dhcp_start(int intf)
{
    ...
    dhcpRx = (DHCP_HDR_T *)((uint64_t)udp + sizeof(UDP_PACKET));
    ...
    if(intf == EMACINTF0)
        memcpy(g_au8MacAddr, mac_addr0, dhcpTx->hw_len);
    else
        memcpy(g_au8MacAddr, mac_addr1, dhcpTx->hw_len);

    memset(g_au8IpAddr, 0, 4);
...
wait_offer:
    ...
    {
        if (retry)
            sysprintf("DHCP DISCOVER retry: %d\n", retry);

        udp_send(intf, g_au8MacAddr, (uint8_t *)g_au8IpAddr,
                 CLIENT_PORT, serverMAC, serverIP, SERVER_PORT,
    ...
    *cptr++ = 4;
    ...
    sysprintf("DHCP REQUEST...\n");

    out = 0;
    for (retry = 0; (retry < 16) && (!out); retry++)
    {
        if (retry)
            sysprintf("DHCP REQUEST retry: %d\n", retry);
...
acked:
    ...
    while (*cptr != 0xff)
    {
        ...
        {
        case 1:                 /* subnet mask */
            sysprintf("Subnet Mask . . . . . . . . . . . : %d.%d.%d.%d\n", cptr[2], cptr[3], cptr[4], cptr[5]);
            break;
        case 2:                 /* time offset */
            break;
        case 3:                 /* router, take 1 */
            sysprintf("Default Gateway . . . . . . . . . : %d.%d.%d.%d\n", cptr[2], cptr[3], cptr[4], cptr[5]);
            break;
        case 4:                 /* time server */
            sysprintf("Time server: %d.%d.%d.%d\n", cptr[2], cptr[3], cptr[4], cptr[5]);
            break;
...
{
    struct sk_buff *rskb = &rxbuf[EMACINTF0];

    if(EMAC0_ReceivePkt(rskb) != 0)
    {
        process_rx_packet(EMACINTF0, (uint8_t *)((u64)(rskb->data)), rskb->len);
    }
}
...
{
    struct sk_buff *rskb = &rxbuf[EMACINTF1];

    if(EMAC1_ReceivePkt(rskb) != 0)
    {
        process_rx_packet(EMACINTF1, (uint8_t *)((u64)(rskb->data)), rskb->len);
    }
}
...
int32_t EMAC0_TransmitPkt(struct sk_buff *ptskb, uint8_t *pbuf, uint32_t len)
{
    ...
    if(ptskb == NULL)
    {
        tskb = (struct sk_buff *)((uint64_t)&txbuf[EMACINTF0] | NON_CACHE);

        tskb->len = len;
        memcpy((uint8_t *)((u64)(tskb->data)), pbuf, len);
        dcache_clean_invalidate_by_mva(tskb->data, (size_t)tskb->len);
        return EMAC_xmit_frames(tskb, EMACINTF0, 1, 0);
    }
    else
    {
        ptskb->len = len;
        return EMAC_xmit_frames(ptskb, EMACINTF0, 1, 0);
    }
}
...
int32_t EMAC1_TransmitPkt(struct sk_buff *ptskb, uint8_t *pbuf, uint32_t len)
{
    ...
    if(ptskb == NULL)
    {
        tskb = (struct sk_buff *)((uint64_t)&txbuf[EMACINTF1] | NON_CACHE);

        tskb->len = len;
        memcpy((uint8_t *)((u64)(tskb->data)), pbuf, len);
        dcache_clean_invalidate_by_mva(tskb->data, (size_t)tskb->len);
        return EMAC_xmit_frames(tskb, EMACINTF1, 1, 0);
    }
    else
    {
        ptskb->len = len;
        return EMAC_xmit_frames(ptskb, EMACINTF1, 1, 0);
    }
}
