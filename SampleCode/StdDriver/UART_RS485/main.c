...
void RS485_HANDLE()
{
    ...
    volatile uint32_t u32IntSts = UART6->INTSTS;
    ...
    else if((u32IntSts & UART_INTSTS_RDAINT_Msk) || (u32IntSts & UART_INTSTS_RXTOINT_Msk) )     /* Rx Ready or Time-out INT*/
    {
        ...
        sysprintf("%2d,",UART6->DAT);
    }
    else if(u32IntSts & UART_INTSTS_BUFEINT_Msk)     /* Buffer Error INT */
    {
        sysprintf("\nBuffer Error...\n");
        UART_ClearIntFlag(UART6, UART_INTSTS_BUFEINT_Msk);
    }
}
...
void RS485_SendAddressByte(uint8_t u8data)
{
    UART_SetLineConfig(UART6, 0, UART_WORD_LEN_8, UART_PARITY_MARK, UART_STOP_BIT_1);
    UART_WRITE(UART6,u8data);
}

void RS485_SendDataByte(uint8_t *pu8TxBuf, uint32_t u32WriteBytes)
{
    UART_SetLineConfig(UART6, 0, UART_WORD_LEN_8, UART_PARITY_SPACE, UART_STOP_BIT_1);
    UART_Write(UART6,pu8TxBuf,u32WriteBytes);
}
...
void RS485_9bitModeMaster()
{
    ...
    UART_SelectRS485Mode(UART6, UART_ALTCTL_RS485AUD_Msk, 0);
...
void RS485_9bitModeSlave()
{
    ...
    UART_SetLineConfig(UART6, 0, UART_WORD_LEN_8, UART_PARITY_EVEN, UART_STOP_BIT_1);

    /* Set RX Trigger Level = 1 */
    UART6->FIFO &= ~UART_FIFO_RFITL_Msk;
    UART6->FIFO |= UART_FIFO_RFITL_1BYTE;
    ...
    /* Set RS485-NMM Mode */
    UART6->MODEM &= ~UART_MODEM_RTSACTLV_Msk;
    UART_SelectRS485Mode(UART6, UART_ALTCTL_RS485NMM_Msk|UART_ALTCTL_ADDRDEN_Msk, 0);
    ...
    UART_SelectRS485Mode(UART6, UART_ALTCTL_RS485AAD_Msk|UART_ALTCTL_ADDRDEN_Msk, MATCH_ADDRSS1);
#endif

    /* Enable RDA\RLS\Time-out Interrupt  */
    UART_ENABLE_INT(UART6, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RLSIEN_Msk));
    ...
    UART_DISABLE_INT(UART6, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RLSIEN_Msk | UART_INTEN_RXTOIEN_Msk));
