...
void SC1_IRQHandler(void)
{
    ...
    if (SC1->INTSTS & SC_INTSTS_TMR0IF_Msk)
    {
        /* Clear interrupt flag */
        SC1->INTSTS = SC_INTSTS_TMR0IF_Msk;
        sysprintf("%d sec\n", sec++);
    }
...
int main(void)
{
    ...
    SC_ENABLE_INT(SC1, SC_INTEN_TMR0IEN_Msk);
    ...
    SC_StartTimer(SC1, 0, SC_TMR_MODE_4, 1000000);  // timer counter will be reloaded.
