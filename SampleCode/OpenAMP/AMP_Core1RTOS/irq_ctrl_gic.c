...
static IRQHandler_t IRQGTimeTable[4] = { 0U };
...
/// GIC's ITARGETSR register controller.
__WEAK void IRQ_SetTarget (IRQn_Type IRQn, uint32_t cpu_target) {
	GIC_SetTarget(IRQn, cpu_target);
}
...
__WEAK int32_t IRQ_SetHandler (IRQn_ID_t irqn, IRQHandler_t handler) {
  ...
  if ((irqn >= 0) && (irqn < (IRQn_ID_t)IRQ_GIC_LINE_COUNT)) {
    ...
    if(irqn == SecPhysicalTimer_IRQn || irqn == NonSecPhysicalTimer_IRQn)
      IRQGTimeTable[(irqn-SecPhysicalTimer_IRQn)+(cpuid()*2)] = handler;
    status =  0;
  } else {
    status = -1;
  }
...
__WEAK IRQHandler_t IRQ_GetHandler (IRQn_ID_t irqn) {
  ...
  if ((irqn >= 0) && (irqn < (IRQn_ID_t)IRQ_GIC_LINE_COUNT)) {
    h = IRQTable[irqn];
    if(irqn == SecPhysicalTimer_IRQn || irqn == NonSecPhysicalTimer_IRQn)
    	h = IRQGTimeTable[(irqn-SecPhysicalTimer_IRQn)+(cpuid()*2)];
  } else {
    h = (IRQHandler_t)0;
  }
...
__WEAK int32_t IRQ_Enable (IRQn_ID_t irqn) {
  ...
  if ((irqn >= 0) && (irqn < (IRQn_ID_t)IRQ_GIC_LINE_COUNT)) {
	GIC_SetTarget((IRQn_Type)irqn, 2);
	GIC_SetSecurity((IRQn_Type)irqn, 0);
...
__WEAK int32_t IRQ_SetMode (IRQn_ID_t irqn, uint32_t mode) {
  ...
  if ((irqn >= 0) && (irqn < (IRQn_ID_t)IRQ_GIC_LINE_COUNT)) {
    ...
    // Check interrupt MA35D0main
    val = mode & IRQ_MODE_MA35D0MAIN_Msk;

    if (val == IRQ_MODE_MA35D0MAIN_NONSECURE) {
      secure = 0U;
    } else {
