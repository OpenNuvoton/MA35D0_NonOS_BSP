...
static uint32_t _rsa_buff_pool[1548] __attribute__((aligned(32)));
static uint32_t *_rsa_buff;

volatile int  g_PRNG_done, g_AES_done, g_AESERR_done;
static volatile int g_HMAC_error, g_HMAC_done;
static volatile int g_RSA_error, g_RSA_done;

static void ECC_Complete(CRPT_T *crpt);
...
void CRYPTO_IRQHandler(void)
{
	...
	{
		// sysprintf("AESERRIF is set!!  0x%x, 0x%x\n", CRPT->INTSTS, CRPT->AES_STS);
		if ((CRPT->AES0_CNT & 0xf) && (CRPT->AES_STS & CRPT_AES_STS_CNTERR_Msk))
		{
			sysprintf("    Not an error. AES_CNT is not multiple of 16.\n");
			g_AES_done = 1;
		}
		else
		{
	...
	if (CRPT->INTSTS & CRPT_INTSTS_RSAEIF_Msk)
	{
		// sysprintf("RSAEIF is set!!\n");
		g_RSA_error = 1;
		CRPT->INTSTS = CRPT_INTSTS_RSAEIF_Msk;
	}
	if (CRPT->INTSTS & CRPT_INTSTS_RSAIF_Msk)
	{
		g_RSA_done = 1;
...
/**
  * @brief  Initialize Crypto Accelerator
  * @return None
  */
void Crypto_Init(void)
{
	...
	IRQ_SetHandler(CRPT_IRQn, CRYPTO_IRQHandler);
	IRQ_Enable(CRPT_IRQn);
...
/**
  ...
  * @return None
  */
void PRNG_Config(CRPT_T *crpt, uint32_t u32KeySize, uint32_t u32SeedReload, uint32_t u32Seed)
{
	if (u32SeedReload)
	{
...
/**
  * @brief  PRNG re-seed
  * @param[in]  crpt   Reference to Crypto module.
  * @param[in]  u32Seed  The new seed.
  * @retval   0     Success.
  * @retval   < 0   Time-out
  */
int PRNG_ReSeed(CRPT_T *crpt, uint32_t u32Seed)
{
	uint64_t t0;

	g_PRNG_done = 0;
	crpt->PRNG_SEED = u32Seed;
	crpt->PRNG_CTL |= crpt->PRNG_CTL | CRPT_PRNG_CTL_SEEDRLD_Msk | CRPT_PRNG_CTL_START_Msk;
...
/**
  ...
  * @param[in]  crpt         Reference to Crypto module.
  * @param[in]  u32EncDec    AES encrypt/decrypt mode select
  *         - \ref AES_MODE_DECRYPT
  *         - \ref AES_MODE_ENCRYPT
  ...
  * @return None
  */
void AES_Open(CRPT_T *crpt, uint32_t u32EncDec, uint32_t u32OpMode,
...
/**
  ...
  * @retval   < 0   Time-out
  */
int AES_Start(CRPT_T *crpt, int is_sm4, uint32_t u32DMAMode)
{
	uint64_t t0;

	crpt->AES_CTL = g_AES_CTL;
	if (is_sm4)
		crpt->AES_CTL |= CRPT_AES_CTL_SM4EN_Msk;

	g_AES_done = g_AESERR_done = 0;
	__DSB();
	crpt->AES_CTL |= CRPT_AES_CTL_START_Msk | (u32DMAMode << CRPT_AES_CTL_DMALAST_Pos);

	t0 = EL0_GetCurrentPhysicalValue();
	while (!g_AES_done && !g_AESERR_done)
	{
...
/**
  ...
  * @retval   < 0   Time-out
  */
int AES_Start_KS(CRPT_T *crpt, uint32_t u32DMAMode, int ksel, int knum)
{
	uint64_t t0;

	if (ksel == 0)
		CRPT->AES_KSCTL = CRPT_AES_KSCTL_RSRC_Msk | knum;     /* from KS SRAM */
	else
		CRPT->AES_KSCTL = (2<<CRPT_AES_KSCTL_RSSRC_Pos) | CRPT_AES_KSCTL_RSRC_Msk | knum; /* from KS OTP */
...
void AES_SetKey(CRPT_T *crpt, uint32_t au32Keys[], uint32_t u32KeySize)
{
	...
	key_reg_addr = (void *)((uint64_t)&crpt->AES0_KEY[0]);
...
void AES_SetInitVect(CRPT_T *crpt, uint32_t au32IV[])
{
	...
	key_reg_addr = (void *)((uint64_t)&crpt->AES0_IV[0]);
...
/**
  ...
  * @param[in]  crpt         Reference to Crypto module.
  * @param[in]  u32FBIAddr   AES feedback in buffer address
  * @param[in]  u32FBOAddr   AES feedback out buffer address
  ...
  * @return None
  */
void AES_SetDMATransfer(CRPT_T *crpt, uint32_t u32FBIAddr, uint32_t u32FBOAddr,
						uint32_t u32SrcAddr, uint32_t u32DstAddr, uint32_t u32TransCnt)
{
	crpt->AES_FBADDR = 0;
	if (u32FBIAddr != 0)
	{
		g_AES_CTL |= CRPT_AES_CTL_FBIN_Msk;
		crpt->AES_FBADDR = u32FBIAddr;
	}
	if (u32FBOAddr != 0)
	{
		g_AES_CTL |= CRPT_AES_CTL_FBOUT_Msk;
		crpt->AES_FBADDR = u32FBOAddr;
	}
	crpt->AES0_SADDR = u32SrcAddr;
	crpt->AES0_DADDR = u32DstAddr;
	crpt->AES0_CNT = u32TransCnt;
}
...
void SHA_Read(CRPT_T *crpt, uint32_t u32Digest[])
{
	...
	reg_addr = (void *)((uint64_t)&(crpt->HMAC_DGST[0]));
	for (i = 0UL; i < wcnt; i++)
	{
...
static void run_ecc_codec(CRPT_T *crpt, uint32_t mode, int enable_scap);

static char  temp_hex_str[160];

int ecc_init_curve(CRPT_T *crpt, E_ECC_CURVE ecc_curve);
...
static void ECC_Complete(CRPT_T *crpt)
{
	...
	if (crpt->INTSTS & CRPT_INTSTS_ECCEIF_Msk)
	{
		...
		// sysprintf("ECCERRIF is set!!\n");
		// sysprintf("ECC_STS = 0x%x\n", crpt->ECC_STS);
		while (1);
	}
}
...
/**
  ...
  */
int  ECC_GeneratePublicKey(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *private_k, char public_k1[], char public_k2[])
{
	int  i, ret = 0;
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GeneratePublicKey_KS(CRPT_T *crpt, E_ECC_CURVE ecc_curve, int k_ksnum, int is_ecdh, char public_k1[], char public_k2[])
{
	int  i, ret = 0;
	...
	if (ret == 0)
	{
		...
		{
			if (is_ecdh)
				CRPT->ECC_KSCTL = (0x2UL<<CRPT_ECC_KSCTL_RSSRCK_Pos) | CRPT_ECC_KSCTL_ECDH_Msk |
							   CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
			else
				CRPT->ECC_KSCTL = (0x2UL<<CRPT_ECC_KSCTL_RSSRCK_Pos) |
							   CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
		}
		else
		{
			if (is_ecdh)
				CRPT->ECC_KSCTL = (0<<CRPT_ECC_KSCTL_RSSRCK_Pos) | CRPT_ECC_KSCTL_ECDH_Msk |
							  CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum);
			else
				CRPT->ECC_KSCTL = (0<<CRPT_ECC_KSCTL_RSSRCK_Pos) |
							  CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum);
		}
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_Mutiply(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char x1[], char y1[], char *k, char x2[], char y2[])
{
	int  i, ret = 0;
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_Mutiply_KS(CRPT_T *crpt, E_ECC_CURVE ecc_curve, int x1_ksnum, char x1[], int y1_ksnum, char y1[],
							int k_ksnum, char *k, char x2[], char y2[])
{
	...
	if (ret == 0)
	{
		...
		if (x1_ksnum >= 0x80)
		{
			ecc_ksxy |= (2<<CRPT_ECC_KSXY_RSSRCX_Pos) | CRPT_ECC_KSXY_RSRCXY_Msk | (x1_ksnum - 0x80);
		}
		else if (x1_ksnum >= 0)
		{
			ecc_ksxy |= (0<<CRPT_ECC_KSXY_RSSRCX_Pos) | CRPT_ECC_KSXY_RSRCXY_Msk | (x1_ksnum);
		}
		else
		{
			Hex2Reg(x1, crpt->ECC_X1);
		...
		if (y1_ksnum >= 0x80)
		{
			ecc_ksxy |= (2<<CRPT_ECC_KSXY_RSSRCY_Pos) | CRPT_ECC_KSXY_RSRCXY_Msk | (y1_ksnum - 0x80);
		}
		else if (x1_ksnum >= 0)
		{
			ecc_ksxy |= (0<<CRPT_ECC_KSXY_RSSRCY_Pos) | CRPT_ECC_KSXY_RSRCXY_Msk | (y1_ksnum);
		}
		else
		{
			Hex2Reg(x1, crpt->ECC_Y1);
		}
		crpt->ECC_KSXY = ecc_ksxy;

		if (k_ksnum >= 0x80)
		{
			crpt->ECC_KSCTL = (2<<CRPT_ECC_KSCTL_RSSRCK_Pos) |
							   CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
		}
		else if (k_ksnum >= 0)
		{
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSecretZ(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *private_k, char public_k1[], char public_k2[], char secret_z[])
{
	int  i, ret = 0;
...
/**
  ...
  */
int  ECC_GenerateSecretZ_KS(CRPT_T *crpt, E_ECC_CURVE ecc_curve, int k_ksnum, char *private_k,
							...
							int z_to_ks, int z_owner, char secret_z[])
{
	...
	if (k_ksnum >= 0x80)
	{
		ecc_ksctl |= (0x2<<CRPT_ECC_KSCTL_RSSRCK_Pos) |
						   CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
	}
	else if (k_ksnum >= 0)
	{
		ecc_ksctl |= (0<<CRPT_ECC_KSCTL_RSSRCK_Pos) |
						  CRPT_ECC_KSCTL_RSRCK_Msk | (k_ksnum);
	}
#if 0
	...
	Hex2Reg(public_x, crpt->ECC_X1);
	Hex2Reg(public_y, crpt->ECC_Y1);

	if (z_to_ks)
	{
		ecc_ksctl |= (z_owner << CRPT_ECC_KSCTL_OWNER_Pos) | CRPT_ECC_KSCTL_WDST_Msk;
	}

	crpt->ECC_KSCTL = ecc_ksctl;
	...
	sysprintf("ECC_KSCTL = 0x%x\n", ecc_ksctl);

	//    dump_ecc_reg("CRPT_ECC_POINT_X1", crpt->ECC_X1, 10);
	//    dump_ecc_reg("CRPT_ECC_POINT_Y1", crpt->ECC_Y1, 10);
	//    dump_ecc_reg("CRPT_ECC_CURVE_K", crpt->ECC_K, 10);
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSignature(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *message,
							   char *d, char *k, char *R, char *S)
{
	...
	if (ret == 0)
	{
		...
		run_ecc_codec(crpt, ECCOP_POINT_MUL, 1);
		...
		run_ecc_codec(crpt, ECCOP_MODULE | MODOP_ADD, 0);
		...
		run_ecc_codec(crpt, ECCOP_MODULE | MODOP_DIV, 0);
		...
		run_ecc_codec(crpt, ECCOP_MODULE | MODOP_MUL, 0);
...
int  ECC_Write_N(CRPT_T *crpt, E_ECC_CURVE ecc_curve)
{
	int   i;
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSignature_KS(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *message,
							   int d_ksnum, int k_ksnum, char *R, char *S)
{
	...
	if (ret == 0)
	{
		...
		run_ecc_codec(crpt, ECCOP_POINT_MUL | CRPT_ECC_CTL_ECDSAR_Msk, 1);
		CRPT->ECC_KSCTL = 0;
		 ...
		 *      (4) Set CURVEM(CRPT_ECC_CTL[31:22]) to key length
		 *      (5) Set ECCOP(CRPT_ECC_CTL[10:9]) to 01
		 *      (6) Set ECDSAS(CRPT_ECC_CTL[4]) to 1
		 *      (7) Set START(CRPT_ECC_CTL[0]) to 1
		 *      (8) Wait for BUSY(CRPT_ECC_STS[0]) be cleared
		...
		ksxy = CRPT_ECC_KSXY_RSRCXY_Msk;

		if (k_ksnum >= 0x80)
		{
			ksxy |= (2<<CRPT_ECC_KSXY_RSSRCX_Pos) | (k_ksnum - 0x80);
		}
		else if (k_ksnum >= 0)
		{
			ksxy |= (0<<CRPT_ECC_KSXY_RSSRCX_Pos) | (k_ksnum);
		}
		else
			return -3;
		...
		if (d_ksnum >= 0x80)
		{
			ksxy |= (2<<CRPT_ECC_KSXY_RSSRCY_Pos) | ((d_ksnum - 0x80) << 8);
		}
		else if (d_ksnum >= 0)
		{
			ksxy |= (0<<CRPT_ECC_KSXY_RSSRCY_Pos) | (d_ksnum << 8);
		}
		else
		{
			return -3;
		...
		/* 4-(4) Set CURVEM(CRPT_ECC_CTL[31:22]) to key length  */
		/* 4-(5) Set ECCOP(CRPT_ECC_CTL[10:9]) to 01            */
		/* 4-(6) Set ECDSAS(CRPT_ECC_CTL[4]) to 1               */
		/* 4-(7) Set START(CRPT_ECC_CTL[0]) to 1                */
		/* 4-(8) Wait for BUSY(CRPT_ECC_STS[0]) be cleared      */

		CRPT_DBGMSG("4-(8) Wait for BUSY be cleared...\n");

		run_ecc_codec(crpt, ECCOP_MODULE | MODOP_DIV | CRPT_ECC_CTL_ECDSAS_Msk, 0);
		crpt->ECC_KSXY = 0;
...
/**
  ...
  */
int  ECC_VerifySignature(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *message,
							 char *public_k1, char *public_k2, char *R, char *S)
{
	...
	if (ret == 0)
	{
		...
		run_ecc_codec(crpt, ECCOP_POINT_ADD, 0);
...
/**
  ...
  * @return  -3   KS error
  */
int  ECC_VerifySignature_KS(CRPT_T *crpt, E_ECC_CURVE ecc_curve, char *message,
							 int x_ksnum, int y_ksnum, char *R, char *S)
{
	...
	if (ret == 0)
	{
		...
		run_ecc_codec(crpt, ECCOP_POINT_MUL, 0);
		...
		if (x_ksnum >= 0x80)
		{
			ksxy |= (2<<CRPT_ECC_KSXY_RSSRCX_Pos) | (x_ksnum - 0x80);
		}
		else if (x_ksnum >= 0)
		{
			ksxy |= (0<<CRPT_ECC_KSXY_RSSRCX_Pos) | (x_ksnum);
		}
		else
			return -3;
		...
		if (y_ksnum >= 0x80)
		{
			ksxy |= (2<<CRPT_ECC_KSXY_RSSRCY_Pos) | ((y_ksnum - 0x80) << 8);
		}
		else if (y_ksnum >= 0)
		{
			ksxy |= (0<<CRPT_ECC_KSXY_RSSRCY_Pos) | (y_ksnum << 8);
		}
		else
		{
			return -3;
...
static int rsa_hex_to_reg(int rsa_len, uint8_t *in, int len, uint32_t *reg)
{
	int dlen = len;
	uint8_t *key, buff[RSA_MAX_BYTE_LEN];
	int i, idx;

	if (dlen > rsa_len)
		return -1;

	memset(reg, 0, RSA_MAX_BYTE_LEN);

	memset(buff, 0, sizeof(buff));

	/* remove leading 0x00's */
	for (key = in; ((*key == 0x00) && (dlen > 0)); key++)
		dlen--;

	if (dlen < 0 || dlen > rsa_len)
		return -1;

	memcpy(&buff[rsa_len - dlen], key, dlen);

	for (idx = rsa_len - 1; idx > 0; idx -= 4)
	{
...
static void rsa_reg_to_hex(u32 *reg, int rsa_len, uint8_t *key, int *key_sz)
{
	int i, idx;

	*key_sz = 0;
	for (idx = rsa_len / 4 - 1 ; idx >= 0; idx--)
	{
		for (i = 3; i >= 0; i--)
		{
			*key++ = (reg[idx] >> (i * 8)) & 0xff;
			(*key_sz)++;
			if (*key_sz >= RSA_MAX_BYTE_LEN)
				return;
		}
	}
}
...
/**
  * @brief  Use RSA engine to encrypt a block of data with given
  *         public key and modulus.
  * @param[in]  crpt     Reference to Crypto module.
  * @param[in]  ras_blen Bit length of RSA keys. It must be one of 1024,
  *                      2048, 3072, or 4096.
  * @param[in]  data     The plain text data.
  * @param[in]  dlen     The length of plain text data in bytes.
  * @param[in]  pubk     The RSA public key.
  * @param[in]  klen     The length of RSA public key in bytes.
  * @param[in]  n        The RSA modulus.
  * @param[in]  nlen     The length of modulus in bytes.
  * @param[out] out      The RSA encrypted output data. The length always
  *                      be <klen> / 8.
  * @param[out] olen     The length of output data.
  * @retval   0    Success.
  * @retval   -1   RSA encrypt time-out
  * @retval   -2   Invalid parameters
  */
int RSA_Encrypt(CRPT_T *crpt, int ras_blen, uint8_t *data, int dlen,
				 uint8_t *pubk, int klen, uint8_t *n, int nlen, uint8_t *out, int *olen)
{
	uint8_t buff[RSA_MAX_BYTE_LEN + 4];
	uint64_t t0;

	if (ras_blen % 1024 || ras_blen > 4096)
		return -2;

	_rsa_buff = nc_ptr(_rsa_buff_pool);
	memset(_rsa_buff, 0, sizeof(_rsa_buff));
	crpt->RSA_SADDR[0] = ptr_to_u32(&_rsa_buff[0]);    /* M */
	crpt->RSA_SADDR[1] = ptr_to_u32(&_rsa_buff[128]);  /* N */
	crpt->RSA_SADDR[2] = ptr_to_u32(&_rsa_buff[256]);  /* E */
	crpt->RSA_SADDR[3] = ptr_to_u32(&_rsa_buff[384]);  /* p */
	crpt->RSA_SADDR[4] = ptr_to_u32(&_rsa_buff[512]);  /* q */
	crpt->RSA_DADDR    = ptr_to_u32(&_rsa_buff[1540]); /* Answer */
	crpt->RSA_MADDR[0] = ptr_to_u32(&_rsa_buff[640]);
	crpt->RSA_MADDR[1] = ptr_to_u32(&_rsa_buff[768]);
	crpt->RSA_MADDR[2] = ptr_to_u32(&_rsa_buff[896]);
	crpt->RSA_MADDR[3] = ptr_to_u32(&_rsa_buff[1024]);
	crpt->RSA_MADDR[4] = ptr_to_u32(&_rsa_buff[1152]);
	crpt->RSA_MADDR[5] = ptr_to_u32(&_rsa_buff[1280]);
	crpt->RSA_MADDR[6] = ptr_to_u32(&_rsa_buff[1408]);
	crpt->RSA_PKADDR   = ptr_to_u32(&_rsa_buff[1536]);

	rsa_hex_to_reg(ras_blen / 8, data, dlen, &_rsa_buff[0]);
	rsa_hex_to_reg(ras_blen / 8, n, nlen, &_rsa_buff[128]);
	rsa_hex_to_reg(ras_blen / 8, pubk, klen, &_rsa_buff[256]);

	g_RSA_error = g_RSA_done = 0;
	__DSB();
	crpt->RSA_CTL = (((ras_blen / 1024) - 1) << CRPT_RSA_CTL_KEYLENG_Pos) | CRPT_RSA_CTL_START_Msk;

	t0 = EL0_GetCurrentPhysicalValue();
	while (!g_RSA_done && !g_RSA_error)
	{
		if (EL0_GetCurrentPhysicalValue() - t0 > 12000000)  /* 1 second timeout */
			return -1;
	}

	rsa_reg_to_hex(&_rsa_buff[1540], ras_blen / 8, buff, olen);

	memcpy(out, &buff[ras_blen / 8 - *olen], *olen);

	return 0;
}

/**
  * @brief  Use RSA engine to decrypt a block of data with given
  *         private key and modulus.
  * @param[in]  crpt     Reference to Crypto module.
  * @param[in]  ras_blen Bit length of RSA keys. It must be one of 1024,
  *                      2048, 3072, or 4096.
  * @param[in]  data     The cipher text data.
  * @param[in]  dlen     The length of cipher text data in bytes.
  * @param[in]  privk    The RSA private key.
  * @param[in]  klen     The length of RSA private key in bytes.
  * @param[in]  n        The RSA modulus.
  * @param[in]  nlen     The length of modulus in bytes.
  * @param[out] out      The RSA encrypted output data. The length always
  *                      be <klen> / 8.
  ...
  * @retval   -2   Invalid parameters
  */
int RSA_Decrypt(CRPT_T *crpt, int ras_blen, uint8_t *data, int dlen,
				 uint8_t *privk, int klen, uint8_t *n, int nlen, uint8_t *out, int *olen)
{
	uint8_t buff[RSA_MAX_BYTE_LEN + 4];
	uint64_t t0;
	...
	rsa_hex_to_reg(ras_blen / 8, data, dlen, &_rsa_buff[0]);
	rsa_hex_to_reg(ras_blen / 8, n, nlen, &_rsa_buff[128]);
	rsa_hex_to_reg(ras_blen / 8, privk, klen, &_rsa_buff[256]);
