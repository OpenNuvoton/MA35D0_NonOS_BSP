...
void ddr_pd(void)
{
	...
	// AXI 4: EMAC0EN, EMAC1EN(CLK_SYSCLS0[27], CLK_SYSCLS0[28])
...
void CA35_DPD(int mode)
{
	...
	if(mode) {
		...
		ddr_pd();
		CLK->PLL[2].CTL1 = 0x21;            // Power MA35D0wn DDR PLL
...
void CA35_NPD(int mode)
{
	...
	//[16]=pd_eanble
	SYS->PMUCR |= (1 << 16);            // Enable Power MA35D0wn
